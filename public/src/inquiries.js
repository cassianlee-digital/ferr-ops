/* 询盘录入（真实弹框 + 持久化）+ 询盘列表加载（ES 模块 · esbuild 打包为 IIFE）。
   loadInquiries 2026-08-26 从 app.js 迁来：它渲染的 renderInqList/refreshInqStats
   ...
   ★ 2026-08-26/27 改版（main）：Hero 左栏「最新询盘」表与 renderInqFeed 已删除，全页只剩一张表——
     表头第二行按列筛选 → 日期倒序连续列表（跨月插分隔行）→ 底部分页；筛选与分页纯前端，不改接口。
     地图跟随筛选结果；顶栏 KPI 与 KPI 页圆环不受筛选影响（考核口径）。筛选条件刻意不持久化。
     跟踪反馈改为 inquiry_feedbacks 多条带时间的记录，格子里每条各占一行、完整不截断。 */

import { esc, openModal, closeModal, toast, tableLoadState } from './ui-kit.js';
import { loadDashboardInq, loadKpiInqDonuts } from './charts.js';
...
// 只认「询盘页」这个 scope —— 时间范围已分页面独立，别页改时间不该连累本页。

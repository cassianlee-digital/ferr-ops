...
function setupAiFooter(){ const foot=document.getElementById('aiModalFoot'); if(!foot)return; foot.className='ai-chat-compose'; foot.style.display='block'; foot.innerHTML='<textarea id="aiChatInput" placeholder="继续追问、补充判断或让 AI 重写成整改动作"></textarea><div class="ai-chat-tools"><label class="btn-ghost" for="aiChatFiles"><i class="ti ti-paperclip"></i> 上传文件/图片</label><input id="aiChatFiles" type="file" multiple accept="image/*,.pdf,.doc,.docx,.xlsx,.csv,.txt" style="display:none"><span id="aiFileList" style="display:flex;gap:6px;flex-wrap:wrap"></span><button type="button" class="btn-ghost" data-ai-command="reanalyze"><i class="ti ti-refresh"></i> 重新分析</button><button type="button" class="btn-ghost" data-ai-command="split"><i class="ti ti-list-check"></i> 拆成整改动作</button><button type="button" class="btn-ghost" data-ai-command="archive"><i class="ti ti-archive"></i> 归档</button><button type="button" class="btn-ghost" data-ai-command="deposit"><i class="ti ti-database-heart"></i> 沉淀</button><button type="button" class="btn-primary" id="aiAdoptBtn" data-ai-command="adopt"><i class="ti ti-clipboard-check"></i> 采纳到整改清单</button><button type="button" class="btn-primary" style="margin-left:auto" data-ai-command="send"><i class="ti ti-send"></i> 发送</button></div>'; foot.onclick=e=>{ const btn=e.target.closest('[data-ai-command]'); if(!btn||!foot.contains(btn))return; const cmd=btn.dataset.aiCommand; if(cmd==='reanalyze')reanalyzeActive(); else if(cmd==='split')splitActions(); else if(cmd==='archive')archiveAiAnalysis(); else if(cmd==='deposit')depositAi(); else if(cmd==='adopt')adoptAi(); else if(cmd==='send')sendAiChat(); }; const files=document.getElementById('aiChatFiles'); if(files)files.onchange=()=>{ const list=document.getElementById('aiFileList'); if(list)list.innerHTML=[...files.files].slice(0,5).map(f=>'<span class="ai-file-chip">'+esc(f.name)+'</span>').join(''); }; }
